...
var Registration  = function (UserRepo, RegistrationRepo) {
    ...
    var checkIfRegistrationExists = function (app) {
        var now = new Date();
        RegistrationRepo.first({email: app.email}).then(function (registration) {
    ...
    var createVerificationToken = function (app) {
        ...
        RegistrationRepo.create(regApp).then(function (newRegApp) {
    ...
    self.on('verification-email-sent', registrationOk);
