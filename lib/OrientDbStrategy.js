...
/**
 * An example strategy for OrientDb using the oriento library. Used for testing
 * @param databaseName
 * @returns {OrientoStrategy}
 * @constructor
 */
var OrientoStrategy = function(databaseName){
    assert.ok(config && config.database, "Server configuration file must be defined");
    assert.ok(databaseName, 'Database name must be defined.');
    var server = Oriento(config.database);
